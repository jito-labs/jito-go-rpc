...
type RPCClient interface {
	...
	Call(ctx context.Context, path string, method string, params ...any) (*RPCResponse, error)
	...
	CallRaw(ctx context.Context, request *RPCRequest, path string) (*RPCResponse, error)
	...
	// to *RPCError.
	//
	CallFor(ctx context.Context, out any, path string, method string, params ...any) error
	...
	CallBatch(ctx context.Context, path string, requests RPCRequests) (RPCResponses, error)
	...
	// - RPCPersponses is enriched with helper functions e.g.: responses.HasError() returns  true if one of the responses holds an RPCError
	CallBatchRaw(ctx context.Context, path string, requests RPCRequests) (RPCResponses, error)

	CallForInto(ctx context.Context, out any, path string, method string, params []any) error
	CallWithCallback(ctx context.Context, path string, method string, params []any, callback func(*http.Request, *http.Response) error) error
	Close() error
}
...
func (client *rpcClient) Call(ctx context.Context, path string, method string, params ...any) (*RPCResponse, error) {
	request := &RPCRequest{
		Method:  method,
		Params:  Params(params...),
		JSONRPC: jsonrpcVersion,
	...
	return client.doCall(ctx, request, path)
}
...
func (client *rpcClient) CallForInto(
	...
	rpcResponse, err := client.doCall(ctx, request, path)
	if err != nil {
		return err
	}
...
func (client *rpcClient) CallRaw(ctx context.Context, request *RPCRequest, path string) (*RPCResponse, error) {
	return client.doCall(ctx, request, path)
}

func (client *rpcClient) CallFor(ctx context.Context, out any, path string, method string, params ...any) error {
	rpcResponse, err := client.Call(ctx, path, method, params...)
	if err != nil {
		return err
	}
...
func (client *rpcClient) CallBatch(ctx context.Context, path string, requests RPCRequests) (RPCResponses, error) {
	if len(requests) == 0 {
	...
	return client.doBatchCall(ctx, path, requests)
}

func (client *rpcClient) CallBatchRaw(ctx context.Context, path string, requests RPCRequests) (RPCResponses, error) {
	if len(requests) == 0 {
		return nil, errors.New("empty request list")
	}
...
func (client *rpcClient) newRequest(ctx context.Context, path string, req any) (*http.Request, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, err
	}

	request, err := http.NewRequestWithContext(ctx, "POST", client.endpoint+path, bytes.NewReader(body))
	if err != nil {
		return request, err
	}
...
func (client *rpcClient) doCallWithCallbackOnHTTPResponse(
	...
	httpRequest, err := client.newRequest(ctx, path, RPCRequest)
	if err != nil {
		if httpRequest != nil {
...
func (client *rpcClient) doBatchCall(ctx context.Context, path string, rpcRequest []*RPCRequest) ([]*RPCResponse, error) {
	httpRequest, err := client.newRequest(ctx, path, rpcRequest)
	if err != nil {
		if httpRequest != nil {
